...
const UserInformation = () => {
  ...
  function validatedValues() {
    ...
    if (!pwdPattern.exec(password)) {
      alert(
        "Password should have at least one uppercase character, one lowercase character, one number and one special symbol"
      );
      return false;
    }
    return true;
  ...
  async function handleClick(event) {
    ...
    const proceed = window.confirm(
      "This action will update your information. Are you sure you want to continue?"
    );
    if (!proceed) {
      return;
    }
  ...
  return (
    <>
      ...
      <button id="updateBtn" onClick={handleClick}>
        Update
      </button>
    </>
  );
